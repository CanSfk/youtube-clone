...
export const Video = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (response.ok) setVideoData(await response.json());
    ...
    <FrontEndLayout>
      {videoData && (
        <div className='flex flex-col gap-3'>
          <video controls src={`${import.meta.env.VITE_BASE_URL}/static/uploads/videos/${videoData.video.video_url}`} className='w-full h-[600px]' />
          <h1 className='text-[20px] text-dark-theme-white'>{videoData.video.video_title}</h1>
          ...
          <div className='flex flex-col gap-6'>
              ...
              <div className='flex gap-4'>
                <div className='w-10 h-10 min-w-9 min-h-9 rounded-full overflow-hidden relative'>
                  <picture className='absolute inset-0'>
                    <source
                      type='image/webp'
                      srcSet={`${import.meta.env.VITE_BASE_URL}/static/uploads/images/sm-${videoData.video.video_cover_image_name}`}
                    />

                    <img
                      src={`${import.meta.env.VITE_BASE_URL}/static/uploads/images/sm-${videoData.video.video_cover_image_name}`}
                ...
                <div className='flex flex-col'>
                  <h2 className='text-dark-theme-white'>{videoData.video.account_name}</h2>
            ...
            <div className='p-3 rounded-lg bg-dark-theme-extra-soft-black'>
              <p className='text-dark-theme-white text-sm'>{videoData.video.video_description}</p>
            </div>
